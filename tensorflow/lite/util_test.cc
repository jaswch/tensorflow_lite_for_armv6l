...
using TestTypes =
    ::testing::Types<std::pair<int32_t, int32_t>, std::pair<uint32_t, uint32_t>,
                     std::pair<int64_t, int64_t>, std::pair<uint64_t, uint64_t>,
                     std::pair<int32_t, uint32_t>, std::pair<uint32_t, int32_t>,
...
TYPED_TEST(CheckedIntTypedTest, MultiplicationUnderflowOppositeSign) {
  ...
  if constexpr (std::is_signed_v<T2>) {
    ...
    EXPECT_TRUE((c * d).Overflow()) << +c.Value() << " * " << +d.Value();
  }
  if constexpr (!std::is_signed_v<T1> && !std::is_signed_v<T2>) {
    GTEST_SUCCEED();
  }
